...
function rangeFor(view, anchor) {
  ...
  if (view === 'month') return { start: new Date(d.getFullYear(), d.getMonth(), 1), end: new Date(d.getFullYear(), d.getMonth() + 1, 1) }
...
export default function MomenceSchedule() {
  ...
  const openSession = async session => {
    ...
    try { setDetail(await api.get(`/api/momence/sessions/${session.id}`)) }
    catch (e) { toast(e.message, 'error') } finally { setDetailLoading(false) }
  }
  const refreshDetail = async () => {
    if (!selected) return
    const data = await api.get(`/api/momence/sessions/${selected.id}`)
  ...
  return <div className="mom-schedule-page">
    <section className="mom-schedule-toolbar">
      <div><span className="eyebrow">Live from Momence</span><h2>Class operations</h2><p>Review every Studio Session, open its roster, and complete front-desk actions.</p></div>
    ...
    {error ? <div className="mom-error card"><AlertTriangle /><div><strong>Schedule could not be loaded</strong><span>{error}</span></div><button className="btn btn-ghost" onClick={load}>Retry</button></div> : loading ? <div className="mom-skeleton-grid">{Array.from({ length: 7 }, (_, i) => <div className="card" key={i} />)}</div> : view === 'list' ? <div className="mom-list card">{sessions.length ? sessions.map(s => <SessionCard key={s.id} session={s} onClick={openSession} />) : <Empty />}</div> : <div className={`mom-calendar mom-calendar-${view}`}>{days.map(day => <section key={dayKey(day)} className={`mom-day card ${sameDay(day, new Date()) ? 'is-today' : ''}`}><header><span>{day.toLocaleDateString('en-IN', { weekday: 'short' })}</span><strong>{day.getDate()}</strong></header><div>{sessions.filter(s => sameDay(s.startsAt, day)).map(s => <SessionCard key={s.id} session={s} onClick={openSession} compact={view === 'month'} />)}{!sessions.some(s => sameDay(s.startsAt, day)) && <span className="mom-no-session">No sessions</span>}</div></section>)}</div>}
...
function SessionDrawer({ session, detail, loading, onClose, onRefresh }) {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => { setSearching(true); try { const r = await api.get(`/api/momence/members?${buildQuery({ query: memberQuery })}`); setMembers(r.members || []) } catch (e) { toast(e.message, 'error') } finally { setSearching(false) } }, 300)
  ...
  const add = (memberId, waitlist = false) => act(`add-${memberId}`, () => api.post(`/api/momence/sessions/${session.id}/bookings`, { memberId, waitlist }), waitlist ? 'Member added to waitlist' : 'Member booked into session')
  ...
  return <div className="mom-drawer-layer" role="dialog" aria-modal="true" aria-label={`Session details for ${session.name}`}><button className="mom-drawer-scrim" onClick={onClose} aria-label="Close session details" /><aside className="mom-session-drawer">
    ...
    {loading ? <div className="mom-drawer-loading">Loading roster…</div> : <>
      <div className="mom-detail-stats"><div><strong>{d.bookingCount ?? active.length}</strong><span>Booked</span></div><div><strong>{d.capacity ?? '∞'}</strong><span>Capacity</span></div><div><strong>{active.filter(b => b.checkedIn).length}</strong><span>Checked in</span></div><div><strong>{d.waitlistBookingCount ?? 0}</strong><span>Waitlist</span></div></div>
      <div className="mom-session-meta"><span><Users />{personName(d.teacher)}</span><span><MapPin />{d.inPersonLocation?.name || 'Online session'}</span></div>
